...
const Dashboard: React.FC = () => {
  ...
  if (!user) {
    // Show a simple loading or redirect could be done here, but for simplicity return null.
    return null;
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <LogOut className="w-4 h-4 mr-1" aria-label="Logout icon" />
                Logout
              </button>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className={`${stat.color} p-3 rounded-lg`}>
                    <Icon className="w-6 h-6 text-white" aria-label={`${stat.label} icon`} />
                  </div>
                </div>
              </div>
            );
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8" data-testid="user-info-card">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
            <User className="w-5 h-5 mr-2" aria-label="User profile icon" />
            Profile Information
          </h3>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
            <Settings className="w-5 h-5 mr-2" aria-label="Settings icon" />
            Quick Actions
          </h3>
